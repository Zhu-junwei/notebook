...
set /p end_date=请输入结束日期，默认（%today%）: 
...
for /f "usebackq tokens=1,* delims=@" %%A in (`powershell -nologo -command ^
		...
		"    Write-Output ($_.urlbase + '_UHD.jpg@' + $_.enddate + '_' + $_.title + '.jpg');" ^
		"}"`
) do (
	set /a bCounter+=1
...
for /L %%i in (%bCounter%,-1,1) do ( 
	...
	if defined w (
		set "imageUrl[%%i]=!imageUrl[%%i]!^&rf=LaDigue_UHD.jpg^&pid=hp^&w=%w%^&h=%h%^&rs=1^&c=4"
	)
	set "savePath=%downloadFolder%\!imageName[%%i]!"
	if not exist "!savePath!" (
		echo 正在下载: !imageName[%%i]!
		curl.exe -L --progress-bar --retry 2 --connect-timeout 10 --max-time 60 -o "!savePath!" "https://www.bing.com!imageUrl[%%i]!"
    ) else (
